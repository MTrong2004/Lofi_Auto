...
## 0. Kết quả rà soát phiên bản 4.9

### 0.1. Lịch sử thay đổi rút gọn

- **4.2:** trạng thái, publish protocol, cache, quyền nhạc và manifest.
- **4.3:** fencing token, codec/encoder, timeline nửa mở và audio segment.
- **4.4:** invalidation/review, verify sau mux và điều phối process tree.
- **4.5:** tách `job_status`/`processing_status`, CFR và idempotency queue.
- **4.6:** installer Stable Diffusion, preflight, resume và rollback.
- **4.7:** OpenAPI động, model lease, extension allowlist và response limits.
- **4.8:** gate triển khai, module contract, Definition of Ready/Done.
- **4.9:** chuẩn hóa Markdown và bố cục đọc/triển khai.

**Nguyên tắc kiểm tra nhất quán:** mọi đầu ra chỉ được dùng khi đồng thời thỏa schema, hash, probe/validator, dependency validity và review bắt buộc. Kiểm tra tĩnh không thể bảo đảm tuyệt đối “không còn lỗi”; release vẫn phải vượt toàn bộ test và gate ở Mục 44–45.

### 0.2. Bản đồ đọc tài liệu

- **Muốn hiểu sản phẩm:** Mục 1–2.
- **Muốn triển khai Core:** Mục 15–20, sau đó Mục 22–38.
- **Muốn tích hợp Stable Diffusion:** Mục 6, Gate G3–G4 tại Mục 16.
- **Muốn kiểm thử/phát hành:** Mục 44–45.
- **Muốn tra cứu schema, lỗi và bảo mật:** Mục 24–29.

### 0.3. Mục lục

<details>
<summary>Mở mục lục đầy đủ</summary>

- [0. Kết quả rà soát](#0-kết-quả-rà-soát-phiên-bản-49)
- [1. Mục tiêu và phạm vi](#1-mục-tiêu-và-phạm-vi-phiên-bản-49)
- [2. Luồng xử lý thống nhất](#2-luồng-xử-lý-thống-nhất)
- [3. Quản lý dự án](#3-quản-lý-dự-án)
- [4. Nhạc, xu hướng và audio](#4-lõi-tra-cứu-và-kiểm-duyệt-nhạc)
- [5. Tạo hình ảnh](#5-lõi-tạo-hình-ảnh)
- [6. Stable Diffusion Local](#6-stable-diffusion-local)
- [7–14. Media pipeline và kiểm thử lõi](#7-tách-ảnh-tự-động-và-parallax-25d)
- [15. Quy ước triển khai](#15-quy-ước-dùng-tài-liệu-để-triển-khai)
- [16. Kế hoạch theo gate](#16-kế-hoạch-triển-khai-theo-gate)
- [17–20. Trách nhiệm và Definition of Ready/Done](#17-ma-trận-file--trách-nhiệm--test)
- [21–38. Hợp đồng nền tảng](#21-thuật-ngữ-và-quy-ước)
- [39–43. Tích hợp, riêng tư và vận hành](#39-capability-registry-và-quản-lý-tích-hợp-bên-ngoài)
- [44. Ma trận kiểm thử](#44-ma-trận-kiểm-thử-hoàn-chỉnh)
- [45. Cổng phát hành](#45-cổng-phát-hành-và-vấn-đề-còn-mở)

</details>

---

## 1. Mục tiêu và phạm vi phiên bản 4.9

Phiên bản 4.9 có một **luồng Core bắt buộc** và các module độc lập:
...
Upload YouTube nằm ngoài phạm vi phiên bản 4.9 cho đến khi toàn bộ luồng tạo nội dung, kiểm duyệt và render hoạt động ổn định.
...
- Ghi JSON theo giao thức nguyên tử ở Mục 23.2; chỉ đổi tên trong cùng filesystem sau khi flush, xác minh, fsync file và sau đó fsync thư mục cha khi nền tảng hỗ trợ.
...
### 6.1. Phạm vi và nguyên tắc

Stable Diffusion Local là module Optional. Cài đặt, cập nhật hoặc lỗi của module này không được làm Core thất bại. App hỗ trợ hai chế độ tách biệt:

1. **Dùng bản đã có:** người dùng chọn thư mục AUTOMATIC1111 hiện hữu; app chỉ kiểm tra, cấu hình adapter và quản lý tiến trình khi người dùng cho phép.
2. **Bản do app quản lý:** app cài vào thư mục riêng từ nguồn và phiên bản đã pin, dùng môi trường Python riêng và có resume/rollback.

Không trộn file giữa hai chế độ. App không nhận quyền sở hữu, không tự cập nhật và không xóa bản cài ngoài do người dùng quản lý.

### 6.2. Cấu hình mục tiêu

- Backend mặc định: AUTOMATIC1111 Stable Diffusion WebUI qua adapter có phiên bản.
- Kiến trúc model khuyến nghị: SD 1.5.
- Batch size: 1.
- Kích thước ưu tiên: 960x540; fallback 768x432 khi OOM.
- Steps mặc định: 20-25.
- Sampler đề xuất: DPM++ 2M Karras hoặc Euler a, nhưng phải lấy từ danh sách sampler API thực tế.
- Startup profile mặc định cho RTX 3050 Ti 4GB: `--api --medvram`.
- Sau một lần OOM đã giải phóng model: chuyển `--lowvram` hoặc giảm kích thước theo policy; không retry vô hạn.
- `--no-half-vae`, `--precision full` và `--no-half` chỉ là fallback cho lỗi tương thích đã nhận diện, không bật mặc định.
- Không chạy job `gpu_heavy` hoặc NVENC cùng lúc; tất cả phải qua scheduler chung.

### 6.3. Nguồn cài và pin phiên bản

Bản do app quản lý chỉ được cài từ manifest phát hành đã duyệt. Manifest phải pin:

```json
{
  "schema_name": "sd_distribution_manifest",
  "schema_version": 1,
  "distribution_id": "a1111_windows_managed",
  "source_repository": "official_repository_identifier",
  "source_type": "git_commit_or_verified_archive",
  "version": "tested_version",
  "commit_or_release": "immutable_identifier",
  "archive_sha256": null,
  "adapter_version": "1.0.0",
  "supported_os": ["windows_10_x64", "windows_11_x64"],
  "supported_python": ["pinned_by_distribution"],
  "tested_at_utc": "..."
}
```
...
- Không cài từ URL tùy ý, nhánh động hoặc script remote chưa pin. Repository/archive phải thuộc allowlist cấu hình và dùng HTTPS; redirect sang host khác phải được đánh giá lại.
- Nếu dùng Git, checkout detached đúng commit đã duyệt, xác minh remote URL và commit tồn tại trong nguồn được phép; không tự `git pull` khi khởi động.
- Nếu dùng archive, kiểm tra SHA-256 trước khi giải nén.
- Chỉ thay manifest bằng bản mới sau integration test và có đường rollback.
- Lưu source, version, commit/hash và adapter version trong `install_state.json`.

### 6.4. Preflight trước cài hoặc nhận bản có sẵn

Preflight phải hoàn tất trước khi tải/cài:

- Xác định OS, kiến trúc 64-bit và đường dẫn hỗ trợ.
- Kiểm tra GPU NVIDIA và driver trước cài; kiểm tra CUDA/PyTorch thực tế sau khi runtime đã tạo. Không báo CUDA failed chỉ vì PyTorch chưa được cài ở bước preflight đầu.
- Kiểm tra RAM và dung lượng: dung lượng gói cài + môi trường + model dự kiến + cache + 20% an toàn.
- Kiểm tra quyền tạo, sửa, đổi tên và xóa file thử trong thư mục đích.
- Chuẩn hóa đường dẫn; chặn path traversal, symlink/junction escape và tên dành riêng.
- Kiểm tra port cấu hình, mặc định 7860; nếu bị chiếm phải xác minh owner hoặc chọn port loopback khác, không tự kill process lạ.
- Kiểm tra không có job GPU/render đang giữ resource lock.
- Kiểm tra Internet chỉ với chế độ cài/tải; `Dùng bản đã có` vẫn phải hoạt động offline.
- Kiểm tra Git/Python theo distribution manifest. Nếu dùng runtime bundled, phải pin nguồn, hash và license của runtime. Không tải Python từ nguồn ngẫu nhiên, không sửa PATH/registry hoặc Python toàn hệ thống.
- Hiển thị trước dung lượng tải, dung lượng cài, nguồn, phiên bản và giấy phép liên quan; chỉ tiếp tục sau khi người dùng đồng ý.

Kết quả preflight: `passed`, `warning`, `failed` hoặc `not_applicable`. Lỗi Optional không chặn Core.

### 6.5. Thư mục và quyền sở hữu

Ví dụ thư mục người dùng chọn:
...
- App Lo-Fi và Stable Diffusion dùng môi trường dependency riêng.
- Bản do app quản lý không cài package vào Python của app Lo-Fi hoặc Python hệ thống.
- Không yêu cầu Administrator nếu không bắt buộc; nếu thao tác cần nâng quyền phải dừng và giải thích rõ.
- Bản có sẵn giữ nguyên thư mục/model/config của người dùng; mọi thay đổi do app phải có backup và provenance.
- Đường dẫn cục bộ lưu trong `config/local_ai.json`; không đưa đường dẫn tuyệt đối vào manifest công khai.

### 6.6. State machine cài đặt
...
```text
failed_retryable | failed_manual_action | cancelling | cancelled
rollback_pending → rolling_back → rolled_back
corrupted | unsupported | external_installation
```

Quy tắc:

- Mỗi bước phải idempotent và ghi checkpoint nguyên tử.
- Phân biệt: `installed` = file/runtime đã có; `running` = process sống; `healthy` = API check đạt; `ready` = healthy + model đã nạp + test generation đạt + resource policy cho phép nhận job. Chỉ `ready` mới nhận job tạo ảnh.
- Hủy chỉ dừng tại ranh giới an toàn; file tải dở hợp lệ có thể giữ `.part` để resume.
- Sau crash, đọc `install_state.json`, xác minh file/hash/process rồi tiếp tục hoặc rollback; không tin trạng thái cũ nếu bằng chứng vật lý không khớp.
- Không xóa bản cài hoạt động cho đến khi staging mới đã health-check thành công.

### 6.7. Metadata cài đặt
...
```json
{
  ...
  "commit_or_release": "immutable_identifier",
  "adapter_version": "1.0.0",
  "install_root": "local_config_reference",
  "staging_path": "staging/release_id",
  "startup_profile": "nvidia_4gb_medvram",
  "configured_port": 7860,
  "bind_host": "127.0.0.1",
  "last_completed_step": "starting",
  "resume_supported": true,
  "rollback_version": "previous_working_version",
  "process_identity": null,
  "active_model_sha256": null,
  "health_report_path": null,
  "health_valid_until_utc": null,
  "last_error_id": null,
  "updated_at_utc": "..."
}
```

File này không chứa token, credential hoặc đường dẫn công khai. Thay đổi phải ghi nguyên tử và có revision/sequence.

### 6.8. Quy trình cài bản do app quản lý

1. Chạy preflight và lấy consent.
2. Tạo `installation_id`, staging và install state.
3. Tải/clone nguồn đã pin vào staging; giới hạn kích thước, timeout và redirect.
4. Xác minh hash/commit và cấu trúc tối thiểu.
5. Tạo môi trường riêng theo distribution manifest.
6. Cài dependency đã pin; log phải lọc credential và URL nhạy cảm.
7. Tạo startup profile mà không sửa launcher gốc nếu có thể; nếu phải sửa, lưu patch và backup. Bản app-managed mặc định vô hiệu extension không nằm trong distribution manifest; extension có thể chạy code nên phải có allowlist, source/hash/license và consent riêng.
8. Chọn hoặc tải checkpoint đã duyệt theo Mục 6.12.
9. Khởi động trên loopback với process identity do app quản lý.
10. Chạy health check và test generation.
11. Sau khi đạt, publish staging thành active installation và ghi capability `available`.
12. Nếu thất bại, giữ bản active cũ; staging chuyển quarantine hoặc rollback theo policy.

Không chạy shell command ghép từ input người dùng. Mọi subprocess dùng argument list, working directory cố định, timeout và process-tree control.

### 6.9. Dùng bản đã có

Khi người dùng chọn `Dùng bản đã có`, app phải:

- Xác minh canonical path, launcher và cấu trúc dự kiến.
- Đọc version/commit nếu có nhưng không tự thay đổi.
- Xác định endpoint, bind host và API capability thực tế.
- Không ghi đè `webui-user.bat`, model hoặc extension nếu chưa có consent riêng.
- Cho phép `connect_only` hoặc `start_stop_allowed`. Chỉ chuyển external install thành `fully_app_managed` qua thao tác import riêng, backup đầy đủ và consent rõ; không tự nâng quyền quản lý.
- Nếu server do người dùng khởi động, app không được tự kill khi đóng.
- Nếu adapter không tương thích, đánh dấu `unsupported` hoặc `degraded`; Core vẫn chạy.

### 6.10. Khởi động và quản lý tiến trình

- Chỉ bind `127.0.0.1` hoặc `::1`; không bật `--listen` trong profile local mặc định.
- Port phải lấy từ config snapshot và capability check, không hard-code tuyệt đối 7860.
- Lưu PID, thời điểm bắt đầu, executable identity, command hash, owner và process group/Windows Job Object.
- `Stop` chỉ dừng process tree đúng installation/job do app quản lý.
- Dừng mềm, chờ grace period, sau đó mới forced kill và ghi log.
- Nếu port có service lạ, không gửi prompt/model data và không tự dừng service đó. Trước request đầu tiên phải đối chiếu process identity/installation ID; HTTP 200 hoặc giao diện giống A1111 chưa đủ để tin cậy.
- Startup có timeout mềm/cứng; khi model đang nạp, progress ở trạng thái không xác định nhưng phải có heartbeat/log tail. Timeout cứng chỉ kill process tree do app sở hữu.

### 6.11. Capability check và tiêu chí Ready

Endpoint tối thiểu cần adapter kiểm tra động:

```text
GET  /openapi.json hoặc /docs để khám phá schema thực tế
GET  /sdapi/v1/options
GET  /sdapi/v1/sd-models
GET  /sdapi/v1/samplers
POST /sdapi/v1/txt2img
GET  /sdapi/v1/progress và POST /sdapi/v1/interrupt nếu instance khai báo hỗ trợ
```

Không coi wiki hoặc tên endpoint là hợp đồng bất biến; adapter phải ưu tiên OpenAPI của chính instance, sau đó xác minh status code, content type, schema, kích thước response và capability trước khi gọi. Endpoint thiếu phải chuyển capability unsupported, không đoán payload.

Chỉ đặt `ready` khi đồng thời đạt:

- Process sống và process identity khớp.
- Socket bind loopback; endpoint không lộ ra interface mạng khác.
- API trả JSON hợp lệ trong timeout; response body bị giới hạn byte, depth và số phần tử để tránh local service lỗi/độc hại làm cạn RAM.
- Có ít nhất một checkpoint được duyệt, đúng hash và tương thích SD 1.5.
- Checkpoint được chọn/nạp thành công và model hash đọc lại từ API/metadata khớp model được yêu cầu. Vì model selection là global state, scheduler phải giữ exclusive model lease trong suốt job.
- Test generation nhỏ, mặc định 256x256, batch 1, tạo được ảnh giải mã hợp lệ.
- Chuỗi base64 phải validate nghiêm ngặt, giới hạn kích thước sau decode, magic bytes đúng định dạng ảnh, kích thước pixel đúng yêu cầu và không rỗng/toàn đen bất thường theo threshold cấu hình.
- Không có OOM, NaN hoặc lỗi decode trong test.
- Start, stop và restart test đạt cho bản app-managed trước khi đánh dấu installation verified.
- Capability registry ghi version adapter, OpenAPI/schema hash, API fields, model hash, startup profile, extension set hash, health-report hash và thời điểm kiểm tra.

Health check nhẹ khi mở app không cần tạo ảnh lại nếu installation/model/startup profile/extension-set hash không đổi và `health_valid_until_utc` chưa hết hạn. Khi đổi model, adapter, startup flags, executable, extension set hoặc API schema hash phải chạy full check lại.

Health report tối thiểu:
...
```json
{
  ...
  "startup_profile_hash": "...",
  "extension_set_hash": "...",
  "active_model_sha256": "...",
  "api_check": "passed",
  "model_load_check": "passed",
  "generation_check": "passed",
  "start_stop_restart_check": "passed",
  "checked_at_utc": "...",
  "valid_until_utc": "..."
}
```

### 6.12. Quản lý checkpoint và tài nguyên model

```json
{
  "schema_name": "model_metadata",
  "schema_version": 1,
  "model_id": "uuid",
  "display_name": "Approved SD 1.5 model",
  "architecture": "sd15",
  ...
  "license": "...",
  "license_url": "...",
  "usage_status": "licensed_for_use",
  "downloaded_at_utc": "...",
  "verification_status": "passed",
  "load_test_status": "passed",
  "minimum_vram_profile": "4gb"
}
```

- Chỉ tải sau consent; hiển thị tên, dung lượng, nguồn, giấy phép và mục đích.
- Tải vào `.part`; resume chỉ khi server hỗ trợ range và identity file không đổi.
- Nếu nhà phát hành cung cấp hash từ kênh tin cậy thì đối chiếu; nếu không, hash tự tạo chỉ chứng minh file không đổi sau tải, không chứng minh nguồn.
- Không nạp pickle/checkpoint không an toàn khi có lựa chọn safetensors; mặc định chỉ nhận `.safetensors` cho model tải qua app.
- Kiểm tra safetensors header/metadata với parser giới hạn tài nguyên, giới hạn kích thước và chuyển file nghi vấn vào quarantine. Không tuyên bố “scan an toàn tuyệt đối”; model chỉ được nạp trong process quyền hạn tối thiểu.
- `verification_status=passed` chưa đủ; phải có `load_test_status=passed` trước khi chọn cho production.
- Không tự tải, cập nhật hoặc thay model giữa lúc project/job đang chạy.

### 6.13. OOM và fallback profile

Trình tự mặc định trên GPU 4GB:

1. Batch 1, 960x540, `--medvram`.
2. Khi OOM: hủy job an toàn, unload model/cache, ghi lỗi và retry tối đa một lần.
3. Nếu vẫn OOM: giảm 768x432.
4. Nếu startup/model load vẫn OOM: chuyển `--lowvram` sau consent hoặc theo policy đã cấu hình.
5. Lỗi VAE NaN/ảnh đen có thể thử `--no-half-vae`; `--precision full --no-half` là phương án cuối vì tăng tài nguyên.
6. Nếu vẫn lỗi: đánh dấu capability degraded/failed và chuyển ảnh local; không làm project Core failed.

Mọi fallback phải đổi config hash, được ghi trong metadata và buộc health check lại.

### 6.13.1. Khóa model và global state

AUTOMATIC1111 có options/model selection dùng chung cho toàn instance. Vì vậy:

- Mỗi job tạo ảnh phải snapshot `model_sha256`, VAE, sampler, scheduler, width, height, steps, seed và override settings.
- Scheduler chỉ cho một generation job trên instance 4GB; job giữ exclusive lease cho tới khi response đã nhận và model/options được xác minh lại.
- Không cho thao tác UI ngoài app đổi checkpoint giữa job app-managed; nếu phát hiện state drift, hủy publish output và đánh dấu job interrupted.
- Ưu tiên override theo request khi API hỗ trợ; không thay global option nếu không cần.
- Sau timeout/cancel phải poll trạng thái đến khi idle hoặc restart instance app-managed trước khi cấp lease tiếp theo.
- Output metadata lưu model title **và SHA-256**; không dùng tên file làm identity.

### 6.14. Cập nhật và rollback

- Không tự cập nhật khi khởi động hoặc khi có job đang chạy.
- Cập nhật chỉ dùng distribution manifest mới đã pin và được người dùng đồng ý.
- Cài phiên bản mới vào staging; không sửa trực tiếp bản active.
- Backup config do app quản lý và lưu danh sách model/extension; không sao chép secret vào backup thường.
- Chạy migration, API capability check và test generation trên staging.
- Chỉ chuyển con trỏ active sau khi staging đạt; giữ bản cũ trong rollback retention.
- Nếu health check sau chuyển đổi thất bại, tự quay lại bản working trước và ghi rollback event.
- Extension có khả năng thực thi code. Bản app-managed mặc định chỉ nạp extension builtin/allowlisted đã pin hash; extension ngoài manifest bị disabled/quarantine, không tự cài hoặc cập nhật. Bản external có extension lạ chỉ được `connect_only` hoặc degraded cho tới khi người dùng chấp nhận rủi ro.

### 6.15. Hủy, resume, dọn dữ liệu và gỡ cài đặt

- `Cancel` phải dừng download/subprocess của installation đó, không kill process lạ.
- File `.part` giữ lại khi còn resumable và trong retention; file sai hash/MIME chuyển quarantine.
- Staging hỏng không được xóa log/bằng chứng trước khi ghi error record.
- Gỡ bản app-managed phải hiển thị preview: runtime, model, output, cache và asset nào dùng chung.
- Mặc định không xóa model/output người dùng đã chọn; cho phép giữ dữ liệu và chỉ gỡ runtime.
- Bản external chỉ được `disconnect`; app không cung cấp xóa toàn bộ thư mục ngoài.
- Mọi xóa dùng reference check, trash nội bộ nếu có, và log audit.

### 6.16. Lỗi và fallback bắt buộc

- Thiếu Git/Python phù hợp: dùng runtime bundled/pinned nếu distribution hỗ trợ; nếu không, `failed_manual_action`.
- Port bận bởi process lạ: chọn port loopback khác hoặc yêu cầu người dùng xử lý; không kill.
- Download gián đoạn: resume có kiểm tra identity/range; nếu không hỗ trợ thì tải lại sau consent/policy.
- Sai hash hoặc commit: quarantine, không retry vô hạn. Archive phải chống path traversal, symlink entry, file count/size bomb và executable ngoài allowlist.
- Dependency install lỗi: giữ log đã lọc secret, rollback staging và không ảnh hưởng app chính.
- API không tương thích: adapter `degraded`/`unsupported`, không đánh dấu ready.
- Model không nạp được: chặn model đó, cho chọn model khác.
- Stable Diffusion không sẵn sàng: fallback ảnh local hoặc provider khác đã được bật.

### 6.17. Yêu cầu kiểm thử riêng

- Preflight: thiếu GPU, driver lỗi, disk thiếu, path không ghi được, port bị chiếm.
- Cài mới thành công trên Windows 10/11 mục tiêu.
- Crash/kill tại từng state; resume không tải/cài trùng.
- Archive/commit/hash sai và dependency download bị thay đổi.
- Bản external ở chế độ connect-only không bị app sửa hoặc kill; service giả trên cùng port không nhận prompt/model data.
- Process PID bị tái sử dụng; stop không dừng nhầm process.
- API/OpenAPI trả HTML, JSON sai schema, response quá lớn, timeout hoặc endpoint thiếu; interrupt unsupported phải fail an toàn.
- Không có model, model sai kiến trúc, safetensors hỏng, load test lỗi.
- OOM tại load model, 960x540 và test generation; fallback không lặp vô hạn.
- Update staging đạt/thất bại và rollback giữ bản cũ chạy được.
- Uninstall giữ model/output theo lựa chọn và không xóa asset dùng chung.
- Security: path traversal, junction escape, archive traversal/bomb, command injection, malicious extension, oversized JSON/base64 và model-header parser abuse.

### 6.18. Giao diện bắt buộc

- Cài bản do app quản lý.
- Dùng bản đã có.
- Chọn/đổi thư mục.
- Xem preflight và dung lượng dự kiến.
- Xem nguồn, phiên bản, commit/hash và adapter version.
- Cài, hủy, tiếp tục, rollback và gỡ runtime.
- Mở thư mục.
- Khởi động, dừng, restart.
- Kiểm tra API nhanh và health check đầy đủ.
- Chọn/tải/kiểm tra model.
- Xem startup profile, extension trust state và fallback đang dùng.
- Xem tiến độ, bước hiện tại, log đã lọc và hành động đề xuất.

Nút chỉ được bật khi state/capability tương ứng hợp lệ. UI không được hiển thị `Ready` chỉ vì port mở hoặc process đang chạy.
...
Các tên dưới đây là `step` chi tiết của render, không thay thế trạng thái chuẩn tại Mục 22. Trạng thái chuẩn nằm trong `job_status`; bước chi tiết nằm trong `current_step`.
...
- File trung gian chỉ được publish sau verify của chính asset. Video cuối phải được verify lại **sau join và mux audio**, rồi mới publish theo giao thức Mục 38; không coi segment verified là bằng chứng video cuối đạt.
...
## 13. Bản đồ module cấp cao

> **Nguồn chuẩn:** cấu trúc repository tại Mục 15.3. Phần này chỉ mô tả luồng sở hữu, không định nghĩa cấu trúc thư mục thứ hai.
...
Application Services
      ↓
Core Domain + Queue + Scheduler
      ↓
Adapters (FFmpeg, Stable Diffusion, Provider)
      ↓
Workers + Filesystem + SQLite
```

### 13.1. Quy tắc sở hữu

- `app/ui/`: hiển thị và nhận thao tác người dùng.
- `app/controllers/`: chuyển thao tác thành command/query.
- `core/`: quy tắc nghiệp vụ, state machine và dependency invalidation.
- `adapters/`: giao tiếp công cụ/dịch vụ ngoài.
- `workers/`: chạy job nền và giữ resource lease.
- `schemas/`, `requirements/`, `tests/`: nguồn chuẩn cho contract và nghiệm thu.

Không tạo lại các file `step1_*`, `step2_*`, `step3_*`, `step4_*` dạng monolith. Khi migration code cũ, chúng chỉ được dùng làm façade tạm thời và phải có kế hoạch xóa.
...
## 15. Quy ước dùng tài liệu để triển khai

### 15.1. Mức độ bắt buộc

Mỗi yêu cầu phải được hiểu theo một trong bốn mức sau:

- **MUST:** bắt buộc; thiếu hoặc test không đạt thì không được qua gate.
- **SHOULD:** nên làm; có thể bỏ qua khi có lý do kỹ thuật được ghi trong ADR.
- **MAY:** tùy chọn; không được làm Core phụ thuộc vào nó.
- **OPEN:** chưa chốt; không được tự chọn giá trị trong code production.

Các câu dùng “phải”, “không được”, “chỉ khi” tương đương **MUST**. Các câu dùng “nên”, “ưu tiên”, “có thể” tương đương **SHOULD/MAY**. Khi diễn giải mâu thuẫn, thứ tự nguồn chuẩn ở Mục 15.2 được áp dụng.

### 15.2. Nguồn chuẩn và thứ tự ưu tiên

1. JSON Schema và enum trong `schemas/`.
2. Requirement có mã trong `requirements/`.
3. State transition table trong `requirements/state_machine.md`.
4. Acceptance criteria và test trong `tests/` cùng `traceability_matrix.csv`.
5. Config profile có version trong `config/`.
6. Nội dung mô tả còn lại của tài liệu này.

Nếu hai nguồn mâu thuẫn, nguồn ở vị trí cao hơn thắng. Việc sửa phải cập nhật đồng thời requirement, schema, test và migration liên quan. Không để code trở thành nguồn chuẩn duy nhất.

### 15.3. Cấu trúc repository bắt buộc
...
UI chỉ gọi controller/application service. UI không được gọi trực tiếp FFmpeg, SQLite, Stable Diffusion API hoặc filesystem writer.

### 15.4. Hợp đồng chung của module

Mỗi module executable phải khai báo:

```json
{
  "module_id": "stable_diffusion_adapter",
  "module_version": "1.0.0",
  "input_schema": "schemas/sd_generation_request.schema.json",
  "output_schema": "schemas/image_asset.schema.json",
  "error_codes": ["SD_API_TIMEOUT", "SD_MODEL_STATE_DRIFT"],
  "resource_class": "gpu_heavy",
  "idempotency_scope": "project_id+request_hash",
...
Mỗi public operation phải có:

- Input/output schema.
- Precondition và postcondition.
- Error code, retry policy và fallback.
- Resource lock cần giữ.
- Side effect lên database/filesystem.
- Idempotency behavior.
- Cancellation checkpoint.
- Log fields bắt buộc.
- Unit/integration test liên kết requirement.

### 15.5. Quy tắc với giá trị OPEN

- Giá trị OPEN phải tồn tại trong `config/open_values.json`, không rải trong code.
- Production profile không được khởi động module phụ thuộc giá trị OPEN.
- Dev/test profile có thể dùng giá trị thử nghiệm nhưng phải ghi `experimental=true`.
- Mỗi giá trị OPEN có owner, cách đo, deadline quyết định và test xác nhận.
- Khi chốt, tạo ADR, tăng config version và thêm regression test.

Ví dụ:

```json
{
  "key": "sd_generation_timeout_seconds",
  "status": "open",
  "experimental_value": 180,
  "owner": "sd_integration",
  "decision_method": "benchmark_on_target_machine",
  "blocks": ["SDI-002"],
  "experimental": true
}
```
...
## 16. Kế hoạch triển khai theo gate

### 16.1. Gate G0 — Nền móng dữ liệu

**Phạm vi code**

- Schema registry và validator.
- Config loader/versioning.
- Error model.
...
- Schema hợp lệ/không hợp lệ có test.
- Migration có backup và rollback test.
- Fault injection khi ghi file không làm mất bản hợp lệ trước.
- DAT-001, PRJ-001 và test liên kết đạt.

### 16.2. Gate G1 — Project, queue và recovery

**Phạm vi code**

- Project manager.
- Job queue và atomic claim.
- Lock, lease, heartbeat, fencing token.
- State history và recovery manager.
- Process identity/process tree control.

**Exit criteria**

- Hai worker không claim cùng job.
- Stale writer với fencing token cũ bị từ chối.
- Kill worker ở mọi transaction boundary phục hồi được.
- PRJ-002, PRJ-003 và QUE-001 liên quan đạt.

### 16.3. Gate G2 — Core offline đầu cuối

**Phạm vi code**

- Nhập audio local và ảnh local.
- FFprobe/validation.
- Chuẩn hóa audio.
- Ảnh phẳng zoom/pan.
- Render video-only segment.
- Join, mux audio master, verify và manifest.

**Exit criteria**

- Không Internet, không Stable Diffusion vẫn tạo video đạt.
- Cancel/resume không render lại segment hợp lệ.
- Verify sau join/mux đạt.
- E2E Core và recovery test đạt trên máy mục tiêu.

### 16.4. Gate G3 — Stable Diffusion external
...
- Health report.
- Model/global-state lease.
- Generation request/response validation.
- Fallback ảnh local.

**Không thuộc gate này**

- Cài runtime tự động.
- Update A1111.
- Quản lý extension ngoài allowlist.

**Exit criteria**

- Không gửi prompt cho service không xác minh.
- Model state drift làm output invalid.
- Timeout/cancel không dừng nhầm external process.
- SDI-002, SDI-004, SDI-005 và SDI-006 đạt.

### 16.5. Gate G4 — Stable Diffusion app-managed

**Phạm vi code**

- Distribution manifest thật.
- Preflight.
- Staging install.
- Runtime riêng.
- Resume/rollback/uninstall.
- Extension allowlist.

**Exit criteria**

- Install mới, crash-resume, update và rollback đạt.
- Không sửa Python/PATH hệ thống.
- API chỉ bind loopback.
- SDI-001 đến SDI-007 đều có test đạt.

### 16.6. Gate G5 — Enhanced media

- Audio vibe/ambience/loop.
- AI upscale.
- Depth, segmentation, inpainting và Parallax.
- Preview/review/invalidation hoàn chỉnh.

Chỉ bắt đầu khi G2 đạt; tác vụ GPU phải dùng scheduler đã ổn định.

### 16.7. Gate G6 — Optional online và composition

- Trend Hunter.
- Provider online.
- Sáng tác nhạc.

Mỗi integration phải có capability adapter, mock test, quota/rate-limit test, privacy disclosure và fallback độc lập.

### 16.8. Quy tắc chuyển gate

- Không chuyển gate nếu còn test bắt buộc failed hoặc requirement không có test.
- Lỗi Critical/High chặn gate.
- Lỗi Medium cần workaround, owner và deadline.
- Mỗi gate tạo build/tag có version để rollback.
- Không thực hiện module ở gate sau bằng cách bypass abstraction chưa ổn định của gate trước.

---

## 17. Ma trận file — trách nhiệm — test

| Thành phần | Trách nhiệm duy nhất | Không được làm | Test tối thiểu |
|---|---|---|---|
| `project_manager.py` | Project revision và dependency invalidation | Gọi FFmpeg/API | Unit + recovery |
| `render_queue.py` | Tạo/claim/chuyển trạng thái job | Render media | Concurrency + fault injection |
| `render_worker.py` | Điều phối render theo job snapshot | Sửa config nguồn | Integration + E2E |
| `media_probe.py` | Probe/normalize kết quả công cụ | Quyết định review | Golden fixtures |
| `output_verifier.py` | Kiểm tra output contract | Publish trước pass | Corrupt media tests |
| `sd_installer.py` | State machine cài app-managed | Sinh ảnh | Crash/resume/rollback |
| `sd_process_manager.py` | Start/stop/process identity | Kill external ngoài quyền | PID reuse/security |
| `sd_adapter.py` | API discovery và generation contract | Tự cài runtime/model | Mock schema/timeout |
| `sd_model_manager.py` | Model identity/hash/license/load state | Đổi model ngoài lease | State drift tests |
| `resource_scheduler.py` | Cấp resource lease | Xử lý nội dung media | Race/OOM/cancel |
| `step3_review_app.py` | UI/controller và review command | Logic nặng hoặc direct I/O | UI state tests |

Nếu một file có hơn một trách nhiệm trong bảng, phải tách module trước khi mở rộng tính năng.

---

## 18. Checklist bắt đầu code

### 18.1. Bắt đầu G0/G1

- [ ] Repository và cấu trúc thư mục đã tạo.
- [ ] Coding standard, formatter, linter và test runner đã pin version.
- [ ] Schema project/job/error/state history đã tồn tại.
- [ ] SQLite migration 001 và rollback test đã tồn tại.
- [ ] Requirement ID có trong traceability matrix.
- [ ] CI chạy unit test và schema validation.

### 18.2. Bắt đầu Core G2

- [ ] G1 đạt toàn bộ exit criteria.
- [ ] FFmpeg/FFprobe build đã pin và system check đạt.
- [ ] Render/audio profile không còn giá trị OPEN chặn Core.
- [ ] Bộ fixture audio/ảnh có quyền sử dụng rõ.
- [ ] Golden metadata và verifier thresholds đã version hóa.

### 18.3. Bắt đầu Stable Diffusion G3

- [ ] Schema `sd_generation_request`, `sd_generation_result`, `sd_health_report` hoàn chỉnh.
- [ ] Adapter contract và mock server đã có.
- [ ] Response limits, timeout, interrupt policy và health TTL đã chốt.
- [ ] Model lease/state drift test đã có.
- [ ] External ownership modes đã có test bảo vệ process.

### 18.4. Bắt đầu installer G4

- [ ] Distribution manifest dùng giá trị thật, không còn placeholder.
- [ ] Runtime Python/PyTorch và hash đã chốt.
- [ ] A1111 commit/release đã test trên máy mục tiêu.
- [ ] Extension allowlist đã ký duyệt.
- [ ] Install/update/rollback/uninstall plan có integration test.
- [ ] Dung lượng, timeout, retention và grace period đã chốt bằng benchmark.

---

## 19. Definition of Ready cho một task

Một task chỉ được đưa vào sprint khi có đủ:
...
- Dependency và resource class.
- File/module sở hữu.
- Test case dự kiến.
- Không còn quyết định OPEN chặn task.

Task thiếu một trong các mục trên trả về backlog, không bắt đầu code bằng giả định cá nhân.

---

## 20. Definition of Done cho một task

- Code và review hoàn tất.
- Unit/integration test phù hợp đạt.
- Requirement và traceability matrix cập nhật.
- Không có secret/PII trong log hoặc fixture.
- Cancel/retry/idempotency/recovery được kiểm tra nếu liên quan.
- Schema/migration/config version được tăng nếu thay đổi contract.
- Có regression test cho lỗi đã sửa.
- Tài liệu chỉ cập nhật phần contract/hành vi thay đổi; không sao chép mô tả sang nhiều nơi.

---

## 21. Thuật ngữ và quy ước
...
## 22. Mô hình trạng thái thống nhất

### 22.1. Trạng thái workflow theo module
...
### 22.3. Quy tắc chuyển trạng thái
...
## 23. Nguồn trạng thái, khóa ghi và phục hồi
...
## 24. Schema và hợp đồng dữ liệu

### 24.1. Quy tắc bắt buộc
...
### 24.2. Danh sách schema tối thiểu
...
## 25. Chuẩn lỗi, retry và fallback
...
### 26.4. File/media không đáng tin
...
### 26.7. Model và tài nguyên âm thanh
...
## 27. Cache và ma trận vô hiệu hóa

### 27.1. Thành phần hash
...
### 27.3. Đồ thị nguồn gốc và phụ thuộc
...
## 28. Quyền sử dụng và bằng chứng
...
### 28.4. Sáng tác và tương đồng nội bộ
...
## 29. Kiểm tra chất lượng định lượng
...
## 30. Đặc tả render phân đoạn

### 30.1. Tham số phải đồng nhất
...
### 30.3. Timeline và biên audio/video
...
## 32. Cấu hình, log và chẩn đoán hệ thống

### 32.1. Cấu hình tập trung
...
- Stable Diffusion installation state, ownership mode, pinned version/commit, runtime, bind address, API capability, checkpoint hash và test generation nếu tính năng được bật.
...
## 33. Quy tắc đặt tên và quản lý file
...
## 34. Dọn dữ liệu dựa trên tham chiếu
...
## 35. Mã yêu cầu và truy vết kiểm thử
...
### 35.2. Yêu cầu nền tảng bắt buộc
...
- `SDI-001`: bản app-managed chỉ cài từ distribution manifest đã pin và xác minh.
- `SDI-002`: không đánh dấu `ready` trước khi API, model load và test generation đạt.
- `SDI-003`: update phải qua staging và có rollback về bản working trước.
- `SDI-004`: app không sửa, xóa hoặc dừng bản external ngoài quyền quản lý đã được người dùng cấp.
- `SDI-005`: adapter phải khám phá/xác minh API schema thực tế và giới hạn response trước khi xử lý.
- `SDI-006`: model/global options phải được khóa theo job; state drift làm output invalid.
- `SDI-007`: extension app-managed chỉ được nạp từ allowlist đã pin; extension lạ không được tự chạy.
...
## 36. Tiêu chí sẵn sàng bắt đầu code

Chỉ bắt đầu triển khai **Core nền tảng** khi đạt các điều kiện dưới đây. Module nào còn giá trị ở Mục 45.3 chưa chốt thì chưa được code hành vi phụ thuộc giá trị đó:
...
Tuân theo các gate tại Mục 16; không duy trì danh sách thứ hai.
---

## 37. Tiêu chí hoàn thành phiên bản 4.9

Phiên bản 4.9 chỉ được coi là hoàn thành khi:
...
- Không còn task production thiếu Definition of Ready; mọi module có contract, owner, requirement và test mapping.

---

## 38. Idempotency, giao dịch và tính nhất quán
...
## 39. Capability registry và quản lý tích hợp bên ngoài
...
## 40. Quyền riêng tư, lưu giữ và xuất dữ liệu
...
## 41. Backup, khôi phục và kiểm tra thảm họa
...
## 42. Cập nhật, phụ thuộc và chuỗi cung ứng
...
## 43. Hiệu năng, giới hạn và khả năng đáp ứng
...
## 44. Ma trận kiểm thử hoàn chỉnh
...
- SD installer: preflight, pinned source, staging, resume, health check, update và rollback.

### 44.3. End-to-end test

- Audio local + ảnh local → asset video `verified`, output job `completed` và manifest `published`.
- Stable Diffusion app-managed/external → health check → ảnh; khi lỗi → fallback ảnh phẳng.
- Audio preview → duyệt → render phân đoạn → resume → manifest.

### 44.4. Fault-injection test

- Kill worker tại từng ranh giới transaction và giữa claim queue/ghi lease/publish asset.
- Mất mạng, hết quota, file tải thiếu, hash sai, crash giữa các bước cài SD và rollback thất bại có kiểm soát.
...
- Path traversal, symlink/junction/archive escape, tên file độc hại, command injection và process ownership khi quản lý SD.
- URL redirect, URL nội bộ, MIME giả và file nén/binary bất thường.
- Token redaction trong log, export và thông báo lỗi.

### 44.6. Regression và golden test
...
## 45. Cổng phát hành và vấn đề còn mở

### 45.0. Phân loại mức độ lỗi
...
- Installer có thể chạy nguồn/extension chưa pin, sửa Python hệ thống, bind API ra mạng, tin nhầm service trên port, xử lý response không giới hạn, đánh dấu ready không qua test generation, để model state drift hoặc update không rollback được.
...
### 45.2. Definition of Done cho mỗi yêu cầu
...
### 45.3. Vấn đề phải chốt trước khi code module tương ứng
...
- Distribution manifest chính thức cho AUTOMATIC1111: source, commit/release, runtime Python/PyTorch, adapter version, OpenAPI schema hash, extension allowlist và bộ test trên Windows 10/11 với RTX 3050 Ti 4GB.
